...
resource "null_resource" "run_jmeter_loadtesting" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo /var/lib/apache-jmeter-5.3/bin/jmeter -n -t /home/${var.user}/${basename(var.source_jmx[count.index])} -Dserver.rmi.ssl.disable=true -R ${var.slave_ips}  -l /usr/share/nginx/html/logfile-${time_static.this.unix}.jtl -e -o /usr/share/nginx/html/${time_static.this.unix}-${count.index}",
    ]
    connection {
      type        = "ssh"
      host        = var.host
